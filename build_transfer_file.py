...
import pickle
def extract_graph(NETWORK_NAME):
    try:
...
def post_process(transfer_file, WALKING_LIMIT):
...
if __name__ == '__main__':
    ...
    if BUILD_TRANSFER==1:
        ...
        CORES = int(input(f"Transfer.txt can be build in parallel. Enter number of CORES (1 for serial). \nAvailable cores (logical and physical):  {multiprocessing.cpu_count()}\n: "))
        ...
        post_process(transfer_file, WALKING_LIMIT)
    else:
        try:
