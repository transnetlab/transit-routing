...
def get_timelist_rrapotr(source, stop_times_file):
...
d_time_list = get_timelist_rrapotr(source, stop_times_file)

'''for source in tqdm(cut_stops):
    for destination in cut_stops:
        if source==destination:continue
        d_time_list = get_timelist_rrapotr(source, stop_times_file)
        fill_in_trips = set(rraptor_main(max_transfer, source, destination,d_time_list,
                                                       routes_by_stop_dict, stops_dict, stoptimes_dict, footpath_dict,
                                                       walking_from_source = 0 , print_para = 0, change_time_sec = 0, optimized= 1))
