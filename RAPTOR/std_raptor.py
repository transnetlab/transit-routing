...
def raptor(SOURCE, DESTINATION, D_TIME, MAX_TRANSFER, WALKING_FROM_SOURCE, CHANGE_TIME_SEC, PRINT_PARA,
    ...
    for k in range(1, MAX_TRANSFER + 1):
        Q.clear()
        marked_stop = list(set(marked_stop))  # removes duplicates
        while marked_stop:
            p = marked_stop.pop()
            try:
        ...
        for route, current_stopindex_by_route in Q.items():
            ...
            for p_i in stops_dict[route][current_stopindex_by_route:]:
                ...
                if current_trip_t == -1 or label[k - 1][p_i] + change_time < current_trip_t[current_stopindex_by_route][
                    1]:  # TODO: Change time should be on the right? because current_trip_t is storing the arrival times right? No. To be discussed.
