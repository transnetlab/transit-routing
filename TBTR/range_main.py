...
for source in tqdm(cut_stops[300:]):
    for destination in cut_stops:
        if source==destination:continue
        d_time_list = get_timelist(source, stop_times_file)
        fill_in_trips = range_TBTR_main(max_transfer, source, destination, d_time_list, routes_by_stop_dict, stops_dict, stoptimes_dict,
                    footpath_dict, trip_transfer_dict, trip_set)
    pd.DataFrame(fill_in_trips).to_csv(f'./fill_in/{source}.csv',index=False)
